use geo::Area;
use geo_overturemaps::{context::{OvertureContext}, model::GersId};
...
#[tokio::test]
async fn test_find_land_cover_in_region() {
    let release_path = "tests/data/release/2025-06-25.0";
    let om = OvertureContext::load_from_release(release_path).await.unwrap();
    ...
    let land_cover = om.find_land_cover_in_region(&geometry).await.unwrap().unwrap();
