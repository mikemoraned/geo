...
@app.cell
def _(maxx, maxy, min_height_slider, minx, miny, mo, plt, prefiltered):
    @mo.cache
    def make_plot(min_height):
        fig, ax = plt.subplots(figsize=(8, 8))
    
        masked = prefiltered[min_height]
...
def _(make_plot, min_height_slider, mo, range_stepsize):
    mo.vstack([
        make_plot(min_height_slider.value),
        mo.hstack([mo.md(f"height >= {min_height_slider.value * range_stepsize:.1f}m"), min_height_slider])
    ], align='center')
    return
