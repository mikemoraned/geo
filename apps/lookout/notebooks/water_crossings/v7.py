...
def _(con, overture_glob):
    ...
    # the dataflow handle later cells depend on).
    con.execute(f"""
...
def _(
    ...
    # Step 6 (V2 + V4 + V7): dump each crossing into parts, size each part in a metric CRS, keep
    # the ones we'd notice, then:
    #   V4-dedup: drop centreline POINT crossings that fall inside an areal water polygon.
    #   V7: locate each crossing as a fraction along its rail segment (ST_LineLocatePoint) — this
    #       `frac` is the crossing's Overture Connector position — then drop crossings that sit in a
    #       tunnel/covered stretch (rail_flags whose `between` range covers `frac`), where the water
    #       isn't visible from the train (e.g. underwater rail tunnels around Hamburg).
...
def raw_candidates(crossing_points_count, to_gdf):
    # V7 diagnostics: every raw rail∩water centroid (pre-filter candidates), shown by the test-case
    # viewer (grey) alongside the kept crossings (red) so removed ones are visible.
    _ = crossing_points_count  # dataflow: after the pipeline
...
@app.cell
def test_pick(mo):
    # Pick a test case to visualise (see test_viz.py). The dropdown must be a top-level global for
    # marimo reactivity; changing it redraws the viewer cell below.
...
def test_view(
    ...
    # Focused viewer for the selected test case: water / rail / raw candidates (grey) / kept V7
    # crossings (red) clipped to the bbox, plus a link to the OvertureMaps explorer for that area.
...
def _(cities_gdf, rail_gdf, reps_v5_gdf):
    # Interactive test-case capture (reusable widget lives in bbox_capture.py): pan/zoom so the target
    # fills the map, name it + set expected_crossings, then Append the visible area.
    # grey = rail, red = V5 crossings, dark = cities (hover for name).
    import importlib as _il
    import sys as _sys
    from pathlib import Path as _Path
...
def _(
    ...
    # Preview the captured visible area + in-box crossing count, and append the case on Append click.
    # Separate cell from the widgets (a marimo UI value can't be read in its creating cell); see bbox_capture.
    bbox_capture.result(
        capture_map,
        case_name,
        case_expected,
