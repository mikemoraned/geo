...
/// The projected CRS for German geometry: one zone per country, so every geometry in the
/// country is directly comparable in metres. Generated from PROJ by `just crs-definitions`.
const GERMANY_PROJECTED_PROJJSON: &str = include_str!("etrs89_utm32n.projjson.json");

/// EPSG code of [`GERMANY_PROJECTED_PROJJSON`].
const GERMANY_PROJECTED_EPSG: u16 = 25832;
...
/// A WKB geometry field in the projected CRS appropriate to German geometry
/// ([EPSG:25832](https://epsg.io/25832)), for the metric column a silver dataset carries
/// alongside its lat/lon one.
pub fn projected_wkb_field(name: &str) -> Result<Field, GeoError> {
    let crs = Crs::from_projjson(serde_json::from_str(GERMANY_PROJECTED_PROJJSON)?);
    let metadata = Arc::new(Metadata::new(crs, None));
    Ok(Field::new(name, DataType::Binary, true).with_extension_type(WkbType::new(metadata)))
}

/// Projects lat/lon geometry into the CRS of [`projected_wkb_field`], so distances and
/// lengths come out in metres.
///
...
impl Projector {
    pub fn new() -> Result<Self, GeoError> {
        Ok(Self {
            ...
            to: proj4rs::Proj::from_epsg_code(GERMANY_PROJECTED_EPSG)?,
        })
    }
...
mod tests {
    ...
    fn a_projected_field_carries_the_projected_crs() {
        let field = projected_wkb_field("geometry_utm").unwrap();
