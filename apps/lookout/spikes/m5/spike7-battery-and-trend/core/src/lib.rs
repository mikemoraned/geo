//! Spike 7's behaviour: accumulate NMEA sentences into a position fix, and report the
...
pub enum Event {
    ...
    /// decided here, not there — see [`battery`].
    Battery(u16),
}
    ...
    fn view(&self, model: &Model) -> Self::ViewModel {
        ViewModel {
            ...
            longitude: model
                ...
                .unwrap_or_default(),
            battery: model.battery.charge().map(bars).unwrap_or_default(),
...
mod tests {
    ...
    /// It shares the first line with the clock: eight characters of eight-plus-five.
    #[test]
    fn the_battery_fits_beside_the_clock() {
        ...
        assert_eq!(
            view.clock.chars().count() + view.battery.chars().count(),
            CHARACTERS_PER_LINE
        );
    }
