...
use esp_idf_svc::hal::{
    adc::{
        attenuation,
        oneshot::{AdcChannelDriver, AdcDriver, config::AdcChannelConfig, config::Calibration},
    },
...
use spike7_core::{Effect, Event, Gnss, NEAREST_ON_SCREEN, ViewModel};
...
/// Battery sensing, taken from M5's own board table for `board_M5StickCPlus2` in
/// `M5Unified/src/utility/Power_Class.cpp`: the cell is read on **GPIO38 / ADC1** through a
/// divider that halves it, at 12dB attenuation and 12-bit width. The same table gives GPIO4 as
/// this board's hold pin, which is what spike 0 established on hardware — a useful sign the
/// right board is being read. There is no PMIC to ask (`pmic_adc`), which is the other face of
/// the PLUS2 having no AXP192.
const BATTERY_DIVIDER: f32 = 2.0;
/// Often enough to watch a discharge, rare enough not to crowd the console.
const BATTERY_LOG_INTERVAL_S: u64 = 60;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let crossings = match spike7_core::carried::crossings() {
    ...
    // Calibration turns a raw count into millivolts at the pin using the constants burned
    // into this chip's eFuse, so the reading does not depend on its ADC being nominal. The
    // original ESP32 only offers line fitting — curve fitting is a C3/C6/S3 feature — which is
    // the same fallback M5's own code takes.
    ...
    let mut battery_logged = Instant::now();
    ...
    loop {
        ...
        if ticked.elapsed().as_secs() >= 1 {
            ...
            // Once a second is far more often than a battery changes, but it costs a single
            // conversion and keeps the shell's job to reading the pin. What the voltage means
            // is the core's business.
            match battery_pin.read() {
                Ok(at_pin) => {
                    let millivolts = (f32::from(at_pin) * BATTERY_DIVIDER) as u16;
                    // Logged raw as well as shown as bars: the bars are deliberately coarse,
                    // and checking the divider and calibration are right needs the number.
                    if battery_logged.elapsed().as_secs() >= BATTERY_LOG_INTERVAL_S {
                        battery_logged = Instant::now();
                        log::info!("battery {millivolts}mV ({at_pin}mV at the pin)");
                    }
                    effects.extend(core.process_event(Event::Battery(millivolts)));
                }
                Err(error) => log::warn!("battery read failed: {error}"),
            }
        }
            ...
            match effect {
                Effect::Render(_) => {
                    ...
                    if shown.as_ref() != Some(&view) {
                        // The battery sits at the right-hand end of the clock's line, which is
                        // why the clock is drawn padded to eight rather than the full width.
                        Text::new(
                            &view.battery,
                            Point::new(
                        ...
                        for (line, y) in fix.into_iter().chain(nearest) {
                            // The clock keeps to its own eight characters so the battery
                            // beside it is not overwritten; everything else claims the width.
                            let width = if y == FIRST_LINE_Y {
                                CLOCK_CHARACTERS
                            } else {
                                CHARACTERS_PER_LINE
                            };
                            Text::new(&format!("{line:width$}"), Point::new(MARGIN_X, y), style)
                                .draw(&mut display)
