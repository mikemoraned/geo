...
The points of interest are water crossings, and are called crossings throughout.
...
speed the nearest fix can be over a hundred metres from the crossing, which places several
...
rather than symmetric error. The window starts at 30–120 seconds around a 60-second target,
and is revised from the observed distribution.
...
disregards true negatives. That suits a negative space of every crossing in the region at
every instant. It is arithmetically the Jaccard index, so the standard classification
metrics libraries compute it, and F1 alongside it, from the same per-pair binary labels.
...
outliers that distort a mean. Stratifying by horizon is necessary: a figure averaged across
horizons hides the near-approach region behind a long tail of early estimates that cost
nothing, and the near approach is the only region the notification depends on.
...
**Per-cycle precision and recall over a (crossing × time) space.** The framing is sound: the
imbalance makes precision and recall the right family, over accuracy or ROC. Three
definitional problems make the numbers unresponsive to predictor quality:

- Recall defined as "was any prediction made" is satisfied by construction by a predictor
  emitting for every candidate within its search radius. It measures the relation between
  the search radius and the ground-truth match radius.
- Precision conditioned on an actual crossing instant cannot score a prediction for a
  crossing never reached.
- Per-cycle counting carries the denominator bias described above.
...
- Those metrics assume prediction difficulty falls as the event nears, which does not hold
  where a vehicle can stop without warning close to the crossing. The tolerance band is then
  tightest exactly where error is least reducible, penalising a predictor for behaviour no
  causal predictor can anticipate.
- Prognostic horizon requires an estimate to enter a band and remain inside it, so one late
  perturbation discards an otherwise good approach.
- The event is assumed certain to occur, leaving no place for a candidate crossing that is
  never reached — which here is a principal failure class.

The comprehension cost is also not repaid: the same questions are answered by counting.
...
that is non-monotonic, multi-target, and where the event need not occur at all.
