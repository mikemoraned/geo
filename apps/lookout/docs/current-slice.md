...
- [x] Read the `transport` table; log segments as static `GeoLineStrings` under `/transport/segments` (coloured by subtype) and connectors as `GeoPoints` under `/transport/connectors`, parsing WKB with shapely. Add a transport map pane to the blueprint. *(Coloured by **`class`**, not `subtype` — `subtype` is always `rail`, so `class` (standard_gauge/tram/narrow_gauge/…) is what distinguishes them. WKB parsed with `shapely.from_wkb`, coords flipped from stored `lon lat` to rerun's `(lat, lon)`. `fetch_transport` returns `[]` when the table is absent, so an un-enriched archive still visualises. Live: wrote an .rrd with 2541 transport features across the device tracks.)*
- [x] Add a python test for the new logging path. *(Extracted the WKB→lat/lon transform into a pure `_transport_geometry` so it's testable without a rerun viewer; 8 tests cover fetch (missing/present table), class colouring, coord flip, MultiLineString split, and the `--near` filter.)*
- [x] *Enrichment on top of the unfiltered version:* optionally restrict the segments logged to those within some distance of a gps sample (a `--near <distance>` flag, off by default). **Hack:** filter using a raw distance in **degrees** rather than doing it properly — the correct approach would reproject to a metres-based CRS and measure true distance, but a degrees threshold is good enough for a first cut. Note this caveat in the code/help text. *(`--near <degrees>` filters segments by `shapely` planar distance to a `MultiPoint` of the window's gps fixes; caveat noted in `--near` help + `_transport_geometry` docstring. Live `--near 0.01` shrank the .rrd, confirming it drops far segments.)*
