...
impl Annotated {
    ...
    pub fn rays(&self) -> Vec<MultiLineString> {
        ...
        for group in self.groups.iter() {   
            for geometry in group.collection.iter() {
    ...
    for group in groups.iter() {        
        ...
        for (id, geometry) in group.collection.iter().enumerate() {
            if let Geometry::Polygon(polygon) = geometry {
                ...
                let summary = RegionSummary::new(id, group.name.clone(), centroid.clone(), bucket_width, normalised);
...
pub fn centroids(groups: &[RegionGroup]) -> Vec<Point<f64>> {
    ...
    for group in groups.iter() {        
        let size = group.collection.len();
        console::log_1(&format!("group '{}': calculating centroids for {} geometries", group.name, size).into());
        for entry in group.collection.iter() {
